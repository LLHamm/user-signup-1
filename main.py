...
def validate_element(string_to_validate, element_name):
    error_string = ""
...
def index():
    ...
    if request.method == 'GET':
        ...
        user_name_error = validate_element(user_name, "user name")
        password_error = validate_element(password, "password")
        if verify_password != password: 
            password_verification_error += "passwords do not match."
        if len(email.strip()) > 0:
            email_error = validate_element(email, "email address")    
